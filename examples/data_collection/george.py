...
conf = {
    ...
    'camera_only_samples': 10000, # number of images to capture in camera-only experiment
...
def magnet_current_generator(divisions, images_per_setting):
    ...
    """
    qfd_stride = 100 / divisions
    qdd_stride = 20 / divisions
    
    for qfd_val in np.arange(0, 100 + qfd_stride, qfd_stride):
        for qdd_val in np.arange(0, 20 + qdd_stride, qdd_stride):
            magnet_values = {
                'CA.QFD0880': min(qfd_val, 100),
                'CA.QDD0515': min(qdd_val, 20),
                'CA.DHJ0840': np.random.uniform(-9.0, 9.0),
                'CA.DVJ0840': np.random.uniform(-9.0, 7.0),
            }
    ...
    for experiment in queue:
        ...
        for img in tqdm(experiment['data'], total=experiment['len']):
            ...
            if image is not None:
                ...
                comment = "10Hz repeatition; 8bunch; ND3 filter; testset"
